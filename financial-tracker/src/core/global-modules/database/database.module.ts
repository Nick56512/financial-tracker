...
function createRepositoryProvider<Entity extends PrimaryKeyEntity>(
   ...
   return {
      ...
      useFactory: (dbContext: DbContext) => {
         const creator: IRepositoryCreator<Entity> = new BaseRepositoryCreator<
            Entity,
            EntityRepository<Entity>
         >(dbContext, EntityRepository<Entity>);
...
function createQueryBuilderRepositoryProvider<Entity extends PrimaryKeyEntity>(
   ...
   return {
      ...
      useFactory: (dbContext: DbContext) => {
         const creator: IRepositoryCreator<Entity> = new BaseRepositoryCreator<
            Entity,
            QueryBuilderRepository<Entity>
         >(dbContext, QueryBuilderRepository<Entity>);
         return creator.createRepository(entity);
      },
      inject: [INJECTION_KEYS.DbContext],
   };
         ...
         useFactory: async (configService: ConfigService) => {
            const options: DbConnectOptions = {
               type: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_TYPE,
               ),
               host: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_HOST,
               ),
               port: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_PORT,
               ),
               username: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_USER,
               ),
               password: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_PASSWORD,
               ),
               database: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_NAME,
               ),
               synchronize: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_SYNCHRONIZE,
               ),
               logging: configService.getOrThrow(
                  ConfigurationParameters.DATABASE_LOGGING,
               ),
