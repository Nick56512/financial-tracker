...
export class PaymentsController {
   constructor(
      @Inject(INJECTION_KEYS.PaymentService)
      private readonly paymentService: IUserPaymentsService,
   ) {}
   ...
   public async getSummary(
      ...
      @Query(EndpointsParameters.reportId, new ParseUUIDPipe())
      reportId: string,
   ) {
      ...
      switch (groupBy) {
         case PaymentsSummaryGroupBy.CATEGORY:
            result =
               await this.paymentService.getPaymentsSummaryByCategories(
                  reportId,
               );
            break;
         default:
   ...
   @Delete()
   public async deletePayment(@Query(EndpointsParameters.id) id: string) {
      return {
         success: await this.paymentService.removeById(id),
      };
   }
}
