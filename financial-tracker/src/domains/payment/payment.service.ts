...
import {
  FilterPaymentsQuery,
  PaymentDto,
  PaymentsSummaryByCategories,
  SummaryCategory,
} from './payment.models';

export interface IUserPaymentsService extends IService<PaymentDto> {
  filter(params: FilterPaymentsQuery): Promise<PaymentDto[]>;
  getPaymentsSummaryByCategories(
    reportId: string,
  ): Promise<PaymentsSummaryByCategories | null>;
}

@Injectable()
export class PaymentService
  extends AbstractService<Payments, PaymentDto>
  implements IUserPaymentsService
{
  constructor(
    private readonly paymentRepository: IQueryBuilderRepository<Payments>,
  ...
  async filter(params: FilterPaymentsQuery): Promise<PaymentDto[]> {
    return (await this.modelRepository.filterByMany(params)).map((x) =>
      this.mapper.mapToDto(x),
    );
  }
  async getPaymentsSummaryByCategories(
    reportId: string,
  ): Promise<PaymentsSummaryByCategories | null> {
    ...
    const rows = await this.paymentRepository.sumByField(
      'categoryId',
      'amount',
      ['categoryId'],
      {
