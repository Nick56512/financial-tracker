...
export class PaymentsService extends AxiosHttpService<Payment> implements IGetSummaryByCategories {
    async getSummaryByCategoriesInReport(reportId: string): Promise<PaymentsSummaryByCategories> {
        const result: AxiosResponse<PaymentsSummaryByCategories> = await this.http.get(FinanceApiEndpoints.summary, {
            params: {
                reportId
            }
        })
