...
export class ReportsController {
   constructor(
      ...
      @inject(IoCInjectionKeys.ReportsService)
      private readonly reportsService: IAuthHttpService &
         IHttpService<Report> &
         IFilterHttpService<Report>
   ) {}
   ...
   public chooseReportByUserId(): Scenes.BaseScene<BotContext> {
      ...
      scene.enter(async (ctx) => {
         ...
         const result = await this.reportsService.filter({
         ...
         ctx.reply(
            BotReplies.selectReport,
            Markup.inlineKeyboard(
               result.map((x) => [Markup.button.callback(x.name, `${x.id}`)])
            )
         );
      });
      scene.action(/([0-9a-f\-]{36})/, async (ctx) => {
         ...
         const result = await this.sessionProvider.update(
            ctx.chat.id,
            'currentReportId',
            reportId
         );
