...
import {
   ...
   VerifyCodeResponse,
   SendCodeResponse,
} from '@domains/infrastructure/iverification.service';
...
export class UserAccountService implements IVerificationService {
   ...
   public async sendVerificationCode(email: string): Promise<SendCodeResponse> {
      try {
         const response = await this.http.post(FinanceApiEndpoints.sendCode, {
            email,
         });
         return {
   ...
   public async verifyCode(
      ...
      try {
         const response = await this.http.post(FinanceApiEndpoints.verifyCode, {
            email,
            verificationCode,
         });
